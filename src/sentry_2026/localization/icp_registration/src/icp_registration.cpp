...
IcpNode::IcpNode(const rclcpp::NodeOptions &options)
  ...
  RCLCPP_INFO(this->get_logger(), "raw pcd point size: %ld", cloud->size());
  voxel_refine_filter_.setInputCloud(cloud);
  voxel_refine_filter_.filter(*cloud);
  RCLCPP_INFO(this->get_logger(), "refine pcd point size: %ld", cloud->size());
  ...
  RCLCPP_INFO(this->get_logger(), "icp_registration initialized, %ldms", std::chrono::duration_cast<std::chrono::milliseconds>(time_end - time_start).count());
}
...
void IcpNode::initialPoseCallback(
  ...
  Eigen::Matrix4d map_to_odom = multiAlignSync(cloud_in_, initial_guess);
  // Eigen::Matrix4d map_to_odom = align(cloud_in_, initial_guess);
  if (!success_) {
    map_to_odom = initial_guess;
    RCLCPP_ERROR(this->get_logger(), "ICP failed");
  }

  {
    std::lock_guard lock(mutex_);
    map_to_odom_.transform.translation.x = map_to_odom(0, 3);
    map_to_odom_.transform.translation.y = map_to_odom(1, 3);
    map_to_odom_.transform.translation.z = map_to_odom(2, 3);

    Eigen::Matrix3d rotation = map_to_odom.block<3, 3>(0, 0);
